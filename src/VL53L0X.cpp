...
VL53L0X::VL53L0X(PinName sda, PinName scl, uint8_t addr ) : m_i2c(sda,scl),m_addr(addr), io_timeout(0) // no timeout
  , did_timeout(false)
{
}
...
bool VL53L0X::getSpadInfo(uint8_t * count, bool * type_is_aperture)
{
  ...
  {
    //printf("timeout = %d - iotimeout = %d\r\n", timeout_start_ms, io_timeout);
    if (timeout_start_ms > io_timeout ) { return false; }
    timeout_start_ms++;
  }
  writeReg(0x83, 0x01);
  tmp = readReg(0x92);
